...
本模型将以元界数字身份在元界区块链的行为数据及基础分为基础，通过评分卡模型的打分综合形成数字身份的声誉分数。本声誉模型由基础分和4个维度的分数做组成，基础分为200分（暂定），由于目前元界区块链的生态暂时没有借贷功能及违约选项，MARS模型将根据日后元界区块链的借贷功能及利息功能完善后进行调整，模型公式如下：
