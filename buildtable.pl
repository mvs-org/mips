...
while (++$mipnum <= $topmip) {
	...
	while (<$F>) {
		...
		if ($field eq 'MIP') {
			...
			if (not $found{License}) {
				die "Unacceptable license $val in $fn" unless exists $AcceptableLicenses{$val} or ($val eq 'PD' and exists $GrandfatheredPD{$mipnum});
			}
		} elsif ($field eq 'Comments-URI') {
			if (not $found{'Comments-URI'}) {
				die unless $val eq sprintf('https://github.com/mvs-live/mips/wiki/Comments:MIP-%04d', $mipnum);
			}
	...
	if (not $found{License}) {
		die "Missing License in $fn" unless exists $TolerateMissingLicense{$mipnum};
	}
